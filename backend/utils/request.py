...
# Common loopback addresses. 'localhost' is included as a common alias
# that is not a valid ipaddress literal but is often seen in headers.
...
def get_client_ip():
    ...
    if xri and _is_valid_ip(xri) and not _is_loopback(xri):
    ...
    # 从右向左读取最靠近服务器的可信IP。
    # Security assumption: the rightmost IPs in the chain are the most
    # trusted (closest to the server). If the proxy chain is untrusted,
    # the rightmost IP could still be spoofed — use X-Real-IP instead.
    if xff:
        ips = [ip.strip() for ip in xff.split(',') if ip.strip()]
        for ip in reversed(ips):
            if _is_valid_ip(ip) and not _is_loopback(ip):
                logger.debug(f"Using X-Forwarded-For IP: {ip}")
                return ip

    # 3. 最后回退到直接连接的 IP
    if remote and _is_valid_ip(remote) and not _is_loopback(remote):
